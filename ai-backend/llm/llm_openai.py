...
class OpenAIChatLLM(BaseChatLLM):
    ...
    def __init__(self, api_key: str, base_url: Optional[str] = None, 
        ...
        try:
            self.chat_model = ChatOpenAI(**init_params)
            logger.info(f"OpenAI Chat LLM initialized: model={model_name}, base_url={base_url}")
    ...
    async def generate_async(self, prompt: str, **kwargs) -> str:
        ...
        try:
            ...
            executor = None
            try:
                executor = asyncio.get_event_loop()._default_executor
            ...
            logger.error(f"OpenAI generate_async error: {e}")
            raise
    ...
    async def chat_async(self, messages: List[Dict[str, str]], **kwargs) -> str:
        ...
        try:
            ...
            logger.error(f"OpenAI chat_async error: {e}")
            raise
        ...
        try:
            ...
            def _stream_chat():
                try:
                    ...
                    logger.error(f"OpenAI chat_stream error: {e}")
                    raise
            ...
            while True:
                try:
                    ...
                    logger.error(f"OpenAI chat_stream iteration error: {e}")
                    raise
...
class OpenAIEmbedLLM(BaseEmbedLLM):
    """OpenAI Embedding LLM 实现"""
    
    def __init__(self, api_key: str, base_url: Optional[str] = None, 
                 model_name: str = "text-embedding-ada-002"):
        """
        初始化 OpenAI Embedding LLM
        ...
        try:
            ...
            logger.info(f"OpenAI Embedding LLM initialized: model={model_name}, base_url={base_url}")
        except Exception as e:
            logger.error(f"Failed to initialize OpenAI Embedding LLM: {e}")
            raise
    ...
    async def embed_async(self, text: str) -> List[float]:
        ...
        try:
            ...
            logger.error(f"OpenAI embed_async error: {e}")
            raise


class OpenAIRerankLLM(BaseRerankLLM):
    """OpenAI Rerank LLM 实现（优先使用 rerank API，失败则回退到 embedding）"""
    
    def __init__(self, api_key: str, base_url: Optional[str] = None, 
                 model_name: str = "text-embedding-3-small"):
        """
        初始化 OpenAI Rerank LLM
        
        Args:
            api_key: API Key
            base_url: API 基础 URL（默认使用官方地址）
            model_name: Rerank 模型名称（默认使用 text-embedding-3-small，如果支持 rerank 则使用 rerank 模型）
        """
        self.api_key = api_key
        self.base_url = base_url or "https://api.openai.com/v1"
        self.model_name = model_name
        ...
        logger.info(f"OpenAI Rerank LLM initialized: model={model_name}, base_url={self.base_url}")
    ...
    def rerank(self, query: str, documents: List[str], top_n: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"OpenAI rerank API returned status {response.status_code}, falling back to embedding")
                
            except requests.exceptions.RequestException as e:
                logger.warning(f"OpenAI rerank API request failed: {e}, falling back to embedding")
