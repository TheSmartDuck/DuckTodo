...
class BailianChatLLM(BaseChatLLM):
    """阿里云百炼 Chat LLM 实现（兼容 OpenAI API）"""
    
    def __init__(self, api_key: str, base_url: Optional[str] = None,
                 model_name: str = "qwen-turbo", temperature: float = 0.7,
                 max_tokens: int = 2000):
        """
        初始化百炼 Chat LLM
        ...
        try:
            ...
            logger.info(f"Bailian Chat LLM initialized: model={model_name}, base_url={self.base_url}")
        except Exception as e:
            logger.error(f"Failed to initialize Bailian Chat LLM: {e}")
            raise
    ...
    def chat(self, messages: List[Dict[str, str]], **kwargs) -> str:
        ...
        try:
            ...
            logger.error(f"Bailian chat error: {e}")
            raise
    ...
    async def generate_async(self, prompt: str, **kwargs) -> str:
        ...
        try:
            ...
            logger.error(f"Bailian generate_async error: {e}")
            raise
    ...
    async def chat_async(self, messages: List[Dict[str, str]], **kwargs) -> str:
        ...
        try:
            ...
            logger.error(f"Bailian chat_async error: {e}")
            raise
        ...
        try:
            ...
            def _stream_chat():
                try:
                    ...
                    logger.error(f"Bailian chat_stream error: {e}")
                    raise
            ...
            while True:
                try:
                    ...
                    logger.error(f"Bailian chat_stream iteration error: {e}")
                    raise
...
class BailianEmbedLLM(BaseEmbedLLM):
    """阿里云百炼 Embedding LLM 实现"""
    ...
    def __init__(self, api_key: str, base_url: Optional[str] = None,
        ...
        try:
            ...
            logger.info(f"Bailian Embedding LLM initialized: model={model_name}, base_url={self.base_url}")
        except Exception as e:
            logger.error(f"Failed to initialize Bailian Embedding LLM: {e}")
            raise
    ...
    def embed(self, text: str) -> List[float]:
        ...
        try:
            ...
            logger.error(f"Bailian embed error: {e}")
            raise
    ...
    async def embed_async(self, text: str) -> List[float]:
        ...
        try:
            ...
            logger.error(f"Bailian embed_async error: {e}")
            raise


class BailianRerankLLM(BaseRerankLLM):
    """阿里云百炼 Rerank LLM 实现（使用专门的 rerank API）"""
    ...
    def __init__(self, api_key: str, base_url: Optional[str] = None,
        ...
        Args:
            api_key: API Key
            base_url: API 基础 URL（默认使用百炼地址）
            model_name: Rerank 模型名称（默认使用 gte-rerank-v2）
        """
        self.api_key = api_key
        self.base_url = base_url or "https://dashscope.aliyuncs.com/compatible-mode/v1"
        self.model_name = model_name
        ...
        logger.info(f"Bailian Rerank LLM initialized: model={model_name}, base_url={self.base_url}")
    ...
    def rerank(self, query: str, documents: List[str], top_n: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Bailian rerank API request error: {e}")
            ...
            logger.error(f"Bailian rerank error: {e}")
            raise
